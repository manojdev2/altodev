...
export default function BrandsPage() {
  ...
  const [modal, setModal] = useState<{ open: boolean; item?: Brand }>({ open: false });
  ...
  const fetchBrands = useCallback(async () => {
    ...
    try {
      ...
      toast.error("Failed to load brands. Pull to refresh or retry.");
    } finally {
  ...
  const openEdit = (b: Brand) => { setForm({ name: b.name, image: b.image }); setModal({ open: true, item: b }); };
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      if (modal.item) {
        await api.put(`/admin/brands/${modal.item._id}`, form);
  ...
  if (!bootstrapped) {
    return <PageLoader label="Loading vehicle brands" />;
  }

  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl sm:text-2xl font-bold">Vehicle Brands</h1>
        <button onClick={openAdd} className="flex items-center gap-2 bg-[#00C950] hover:bg-[#00b347] text-white px-3 sm:px-4 py-2 rounded-lg text-sm font-medium transition-colors">
        ...
        <table className="w-full text-sm min-w-[400px]">
          <thead className={`border-b ${dark ? "bg-[#1A2332] border-[#2d3a4a]" : "bg-gray-50 border-gray-200"}`}>
            <tr>
              ...
              <th className={`text-left px-5 py-3 font-medium ${dark ? "text-gray-400" : "text-gray-500"}`}>Name</th>
              <th className={`text-right px-5 py-3 font-medium ${dark ? "text-gray-400" : "text-gray-500"}`}>Actions</th>
            </tr>
          </thead>
          <tbody>
            {tableLoading && (
              <tr><td colSpan={3} className="px-5 py-6 text-center text-gray-400">Refreshing…</td></tr>
            )}
            {!tableLoading && brands.map(b => (
              <tr key={b._id} className={`border-b last:border-0 transition-colors ${dark ? "border-[#2d3a4a] hover:bg-[#253347]" : "hover:bg-gray-50"}`}>
                ...
                <td className="px-5 py-3 text-right space-x-2">
                  <button onClick={() => openEdit(b)} className="text-blue-500 hover:text-blue-700 p-1"><Pencil size={15} /></button>
                  <button onClick={() => del(b._id)} className="text-red-400 hover:text-red-600 p-1"><Trash2 size={15} /></button>
                </td>
              </tr>
            ))}
            {!tableLoading && brands.length === 0 && (
              <tr><td colSpan={3} className="px-5 py-8 text-center text-gray-400">No brands yet.</td></tr>
            )}
          </tbody>
        </table>
        </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className={`rounded-2xl w-full max-w-md p-6 shadow-xl transition-colors ${dark ? "bg-[#1E2A3A] text-white" : "bg-white text-gray-900"}`}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold">{modal.item ? "Edit Brand" : "Add Brand"}</h2>
            ...
            <form onSubmit={save} className="space-y-4">
              ...
              <div>
                <label className={`block text-sm font-medium mb-1 ${dark ? "text-gray-300" : ""}`}>Brand Image</label>
                {form.image ? (
